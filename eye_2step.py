...
def get_target_data(trial, ppd, xpixels, ypixels):
	...
	target_x, target_y, target_t, target2_x, target2_y, target2_t = [None]*6
...
def run_analysis(task, ppd, xpixels, ypixels, sampling_rate, base_path):
	...
	columns = ['task', 'group', 'subject', 'trial_number', 'outcome', 
			   'target_x', 'target_y', 'target_t',
		   	   'target2_x', 'target2_y', 'target2_t',
			   'saccade_x1', 'saccade_y1', 'saccade_t1',
			   'saccade_x2', 'saccade_y2', 'saccade_t2',
			   'saccade_number','duration', 'target_offset', 'amplitude', 'peak_velocity', 'error']
			...
			if outcome in ['Hit','Miss']:
				...
				for saccade,error in zip(saccades,errors):
					...
					data = [[ task, group, subject, trial_num, outcome,
							 ...
							 saccade_num, saccade[6], saccade[7], saccade[8], saccade[9], error]]
...
if __name__ == '__main__':
	...
	# analyze the TSF
	trials, bad_trials = run_analysis('TSF', ppd, xpixels, ypixels, sampling_rate, base_path)
	trials.to_csv('TSF_analyzed.csv', index=False)
